...
def _collect_failures(case: FiveAddressCase, payload: JsonObject) -> list[str]:
    ...
    if (
        ...
        or site.get("municipality") != case.municipality
        or site.get("county") != case.county
    ):
        failures.append("site context changed")
    if (
        not isinstance(comp_search_strategy, dict)
        or comp_search_strategy.get("land_signal_tier") != case.expected_land_signal_tier
    ):
        failures.append("comp search strategy tier changed")
    if (
        not isinstance(comp_search_strategy, dict)
        or comp_search_strategy.get("sales_source_type") != "curated_arcgis"
    ):
        failures.append("sales source type changed")
    if (
        not isinstance(comp_search_strategy, dict)
        or comp_search_strategy.get("exit_comp_source_type") != "curated_arcgis"
    ):
    ...
    if (
        ...
        or guidance.get("recommended_offer") != case.expected_guidance_offer
    ):
        failures.append("acquisition guidance offer changed")
    if (
        not isinstance(guidance, dict)
        or guidance.get("land_value_signal") != case.expected_guidance_land_value
    ):
        failures.append("acquisition guidance land value changed")
    if (
        not isinstance(guidance, dict)
        or guidance.get("land_signal_strength") != case.expected_land_signal_strength
    ):
        failures.append("land signal strength changed")
    if (
        not isinstance(guidance, dict)
        or guidance.get("land_comp_signal_available")
        is not case.expected_land_comp_signal_available
    ):
    ...
    if isinstance(report, dict):
        ...
        underwriting_section = next(
            (
                ...
                if section.get("section_id") == "underwriting_summary"
            ),
            None,
        )
