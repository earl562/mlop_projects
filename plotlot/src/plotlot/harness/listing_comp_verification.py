...
def build_contextual_land_listing_verification(
    *,
    ...
    review_candidates: list[VerifiedListingMetric] = []
            ...
            if address_hint:
                review_candidates.append(
                    VerifiedListingMetric(
                        ...
                        lot_size_sqft=0.0,
                        lot_size_variance_ratio=float(
                            candidate.get("lot_size_variance_ratio") or 1.0
                        ),
    ...
    price_per_acre_values = sorted(metric.price_per_acre for metric in verified_candidates if metric.price_per_acre > 0)
    ...
    return {
        ...
        "verified_candidates": [_listing_metric_payload(metric) for metric in verified_candidates],
        "review_candidate_count": len(review_candidates),
        "review_candidates": [_listing_metric_payload(metric) for metric in review_candidates],
...
def _listing_metric_payload(metric: VerifiedListingMetric) -> JsonObject:
    return {
        "url": metric.url,
        "title": metric.title,
