...
def test_tool_router_enriches_thin_live_comps_with_web_listing_candidates() -> None:
    ...
    assert result.payload["analysis"]["web_listing_search"]["strategy"] == "sold_land_then_improved_sales"
    assert result.payload["analysis"]["web_listing_search"]["selected_search_category"] == "sold_land"
    ...
    assert result.payload["analysis"]["web_listing_candidates"][0]["title"] == "123 Example St sold lot"
    assert result.payload["analysis"]["web_listing_candidates"][0]["address_hint"] == "123 Example St sold lot"
    assert result.payload["analysis"]["web_listing_candidates"][0]["source_domain"] == "www.zillow.com"
    assert result.payload["analysis"]["web_listing_candidates"][0]["classification"] == "likely_vacant_land"
    ...
    assert result.payload["analysis"]["web_listing_candidates"][1]["title"] == "125 Example St sold lot"
...
def test_tool_router_classifies_improved_listing_candidates_as_improved_sales() -> None:
    ...
    with (
        ...
        patch(
            ...
            new=AsyncMock(
                side_effect=[
                    WebSearchResult(status=WebLookupStatus.SUCCESS, provider=WebSearchProvider.EXA, results=[]),
                    WebSearchResult(status=WebLookupStatus.SUCCESS, provider=WebSearchProvider.EXA, results=[]),
                    WebSearchResult(status=WebLookupStatus.SUCCESS, provider=WebSearchProvider.EXA, results=[]),
                    search_result,
                ]
    ...
    assert result.payload["analysis"]["web_listing_search"]["selected_search_category"] == "new_build_houses"
    ...
    assert result.payload["analysis"]["web_listing_candidates"][0]["classification"] == "likely_improved_sale"
...
def test_tool_router_filters_non_zillow_redfin_listing_candidates() -> None:
    ...
    assert result.payload["analysis"]["web_listing_search"]["status"] == "no_usable_listing_candidates"
...
def test_tool_router_loads_rental_market_evidence() -> None:
    ...
    assert result.payload["rental_market_evidence"]["requires_income_assumption_verification"] is True
