...
- [x] **2.2** `domain/steps.py` + `domain/methodology.py` — `KleymanStep` enum (1–8), step→field-key-namespace table, HTN task/method defs as data. Contract: step 5 blocked when `zoning.*` not `verified_fact`. (merged, slice 2.2)
- [x] **2.3** `domain/guardrails.py` — 5 rules (zoning.* must be local_authority; cost.*/cap_rate/financing.* must be assumption; entitlement hypothesis needs next_verification_step; material claim needs evidence_ids; contradictions surface as requires_human_review). Pure functions over `Claim[]`. (slice 2.3)
- [x] **2.4** Move shared types to `domain/types.py` — `ToolContract`/`ToolContext`/`PolicyDecision`/`EvidenceItem`/`ReportClaim` (+ supporting deps `EvidenceCitation`/`SourceType`/`EvidenceConfidence`/`ToolRiskClass`/`EvidenceBackedReportSection`/`DEFAULT_ORDINANCE_LEGAL_CAVEAT`) moved to `domain/types.py`; `land_use/models.py` re-exports them (same identity, not parallel defs) so all existing consumers (`from plotlot.land_use import ...` and `from plotlot.land_use.models import ...`) still work. `land_use/` now thin service module — ordinance/layer query types stay there. 5 new contract tests in `test_domain_types_move.py` pin identity + no-cycle invariant. Existing `test_land_use_tool_contracts.py` still passes. (slice 2.4)
