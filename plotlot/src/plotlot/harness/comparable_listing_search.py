...
def build_comparable_listing_queries(subject_payload: JsonObject) -> list[ComparableListingQuery]:
    ...
    return [
        ComparableListingQuery(
            query=(
                f"{location_terms} sold vacant land lot comps last 6 months "
                f"{domain_fragment}"
            ).strip(),
        ...
        ComparableListingQuery(
            query=(
                f"{location_terms} sold vacant land lot comps last 12 months "
                f"{domain_fragment}"
            ).strip(),
            search_category="sold_land",
            window_months=12,
        ...
        ComparableListingQuery(
            query=(
                f"{location_terms} sold vacant land lot comps last 24 months "
                f"{domain_fragment}"
            ).strip(),
            search_category="sold_land",
            window_months=24,
