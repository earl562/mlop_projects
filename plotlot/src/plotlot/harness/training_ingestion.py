...
def discover_training_video_sources(
    *,
    ...
    return [video for video in videos if _matches_video_filter(video, url=url, category=category)]
...
def build_training_knowledge_index(
    ...
    mappings = {
        mapping.training_concept_id: mapping
        for mapping in map_concepts_to_workflow_templates(concepts)
    }
    return [
        TrainingKnowledgeUnit(
...
def _arv_offer_concept(
    ...
    return TrainingConcept(
        ...
        formulas=[
            "max_offer = indicated_arv - repair_costs - closing_costs - selling_costs - target_profit"
        ],
        assumptions=[
            "comps are similar enough to support ARV",
            "repair and sale costs are explicit",
        ],
...
def _general_development_concept(
    ...
    return TrainingConcept(
        ...
        extracted_facts={
            "source_lane": "rehabvaluator_fixture",
            "calculator": "residual_land_value",
        },
        extracted_steps=[
            "run density study",
            "estimate costs",
            "value as built",
            "solve residual land value",
        ],
