...
from outreach.tools.gmail import send_email
...
async def run_email_campaign(session: AsyncSession, dry_run: bool = False) -> list[dict]:
    """
    ...
    for row in rows:
        ...
        send_result = await send_email(prospect.email, subject, body)
