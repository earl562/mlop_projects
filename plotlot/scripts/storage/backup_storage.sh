...
: "${STORAGE_OBJECT_DIR:?STORAGE_OBJECT_DIR is required}"
...
tar -C "$STORAGE_OBJECT_DIR" -cf "$work_dir/objects.tar" .
...
tar -C "$work_dir" -cf - database.dump objects.tar manifest.json |
