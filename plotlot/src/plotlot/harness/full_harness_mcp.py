...
class FullHarnessMCPAdapter:
    ...
    async def call_tool_async(self, request: FullHarnessMCPToolCallRequest) -> HarnessToolCallResult:
    ...
    return JSON_OBJECT_ADAPTER.validate_python(
        {
            ...
            "sources": [
                ...
                *[_contract_json(item) for item in load_cost_assumption_source_catalog(SourceMode.FIXTURE)],
            ],
        }
    )
...
def _is_router_result_awaitable(result: RouterCallResult) -> TypeGuard[Awaitable[HarnessToolCallResult]]:
