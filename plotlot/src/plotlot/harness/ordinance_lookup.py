...
def _municipality_match_rank(requested: str, candidate: str) -> int:
    ...
    if requested_norm == candidate_norm:
        return 3
    if requested_norm and candidate_norm and (
        requested_norm in candidate_norm or candidate_norm in requested_norm
    ):
        return 2
...
def _rank_search_result(result: SearchResult, *, municipality: str, zone_code_boost: str | None) -> tuple[int, int, float]:
...
def _evidence_item(
    *,
    ...
    citation = ordinance_citation(
        ...
        path=[p for p in [getattr(result, "chapter", None), result.section] if p],
        raw_text_for_hash=(
            f"{args.municipality}:{result.section}:{result.section_title}:"
            f"{result.chunk_text[:300]}"
        ),
    )
