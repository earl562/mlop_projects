...
def _auth() -> JurisdictionSourceAuthority:
    return JurisdictionSourceAuthority(
        state="FL", county="Broward", municipality="Fort Lauderdale",
        jurisdiction_type=JurisdictionType.MUNICIPALITY, authority_scope=AuthorityScope.ZONING,
        provider=Provider.MUNICODE,
        canonical_url="https://x", source_url="https://x", source_title="Test Code",
        official_status=OfficialStatus.PUBLISHER_COPY, legal_caveat="verify with municipality",
    )
...
class TestRunIngestion:
    ...
    async def test_ingests_html_with_table_preserves_chunk_kind(self):
        ...
        table_chunks = [c for c in result.chunks if c.chunk_kind == TableKind.DIMENSIONAL_TABLE.value]
    ...
    async def test_idempotent_unchanged_source_no_duplicate_chunks(self):
        ...
        async def fetcher(url: str) -> tuple[int, str]:
            return 200, html
        prior_snapshot = None
        first = await run_ingestion(authority=_auth(), fetcher=fetcher, prior_snapshot=prior_snapshot)
        # re-ingest with the now-stored snapshot as prior — must be unchanged.
        second = await run_ingestion(authority=_auth(), fetcher=fetcher, prior_snapshot=first.snapshot)
    ...
    async def test_quality_score_computed(self):
        async def fetcher(url: str) -> tuple[int, str]:
            return 200, "<html><table><tr><th>District</th><th>Density</th></tr><tr><td>RS-8</td><td>8.0</td></tr></table></html>"
        result = await run_ingestion(authority=_auth(), fetcher=fetcher, prior_snapshot=None)
        assert result.quality_score is not None
        assert 0.0 <= result.quality_score <= 1.0
        assert IngestionEventType.JURISDICTION_QUALITY_SCORED.value in {e.type for e in result.events}
