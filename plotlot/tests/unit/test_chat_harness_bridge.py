...
from plotlot.api.chat_harness_bridge import (
    compact_harness_active_analysis,
...
async def test_execute_full_harness_chat_tool_runs_shared_deal_analysis_with_live_context(
    ...
    assert payload["evaluation_status"] == "blocked"
    assert payload["active_analysis"]["status"] == "blocked"
    assert "valuation" not in payload["active_analysis"]
...
async def test_compact_harness_active_analysis_surfaces_harness_comp_trust_fields(
    ...
    artifacts["property_record"] = dict(artifacts["property_record"]) | {
    ...
    assert payload["active_analysis"]["folio"] == "74434321060170150"
    assert payload["active_analysis"]["owner"] == "OWNER OF RECORD"
    ...
    assert valuation["land_value_range"] == [150000.0, 165000.0]


@pytest.mark.asyncio
async def test_compact_harness_active_analysis_hides_valuation_when_evaluation_is_blocked(
    monkeypatch,
    harness_store_path,
) -> None:
    base_result = await run_fixture_deal_analysis_async(
    ...
    artifacts["evaluation_readiness"] = {
        "status": "blocked",
        "can_recommend": False,
        ...
        "allowed_outputs": ["parcel_identity", "preliminary_zoning_context", "evidence_gap_plan"],
        "next_steps": ["Retrieve the controlling municipal ordinance section."],
    }
    artifacts["comps"] = {
    ...
    payload = json.loads(
        await execute_full_harness_chat_tool(
            ...
            context=ToolContext(
                workspace_id="ws_live",
                actor_user_id="chat_agent",
                run_id="run_blocked_analysis",
                live_network_allowed=True,
            ),
            session_id="sess_blocked_analysis",
        )
    )
    ...
    assert payload["active_analysis"]["status"] == "blocked"
    assert "valuation" not in payload["active_analysis"]
    assert payload["active_analysis"]["evaluation_readiness"]["can_recommend"] is False
    assert payload["events"] == []
    assert payload["payload"]["artifacts"]["evaluation_readiness"]["status"] == "blocked"
    assert "comps" not in payload["payload"]["artifacts"]
    assert "calculations" not in payload["payload"]
    assert "282805" not in json.dumps(payload)
    assert "653589" not in json.dumps(payload)


def test_compact_live_analysis_fails_closed_when_readiness_is_missing() -> None:
    payload = {
        "source_mode": "live",
        "artifacts": {
            "property_record": {
    ...
    compact = compact_harness_active_analysis(payload)

    assert compact is not None
    assert compact["status"] == "blocked"
    assert "valuation" not in compact
