...
def build_acquisition_decision(
    ...
    comp_floor = inputs.comps.valuation_low if inputs.comps.status == CompStatus.QUALIFIED else None
    residual_ceiling = (
        inputs.residual_land_value
        if inputs.residual_land_value is not None and inputs.residual_land_value > 0
        else None
    )
    ...
    if comp_floor is None or residual_ceiling is None:
        ...
        if residual_ceiling is None:
            reasons.append("A positive deterministic residual land-value ceiling is unavailable.")
            questions.append("Complete deterministic feasibility and residual underwriting.")
    ...
    if cushion_dollars < -tolerance:
        ...
        reasons = (
            ...
            "Advance means human diligence is warranted; it is not a purchase instruction.",
        )

    cushion_percent = cushion_dollars / supported_basis * 100 if supported_basis > 0 else None
    return AcquisitionDecision(
        status=status,
        purchase_price=inputs.purchase_price,
        comp_floor=comp_floor,
        ...
        cushion_percent=(round(cushion_percent, 2) if cushion_percent is not None else None),
