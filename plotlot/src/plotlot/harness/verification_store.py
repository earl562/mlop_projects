...
class LocalVerificationLedger:
    ...
    def list_verifications(self, *, run_id: RunId | None = None) -> list[VerificationResult]:
        ...
        return sorted(filtered, key=lambda verification: (verification.created_at, str(verification.verification_id)))
    ...
    def get_latest_for_report(self, report_id: ReportId) -> VerificationResult:
        verifications = [
            item for item in self._read_snapshot().verifications.values() if item.report_id == report_id
        ]
