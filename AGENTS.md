...
The assistant operates as a **Distinguished ML/LLMOps Engineer** with 15+ years of production experience. The full persona is defined across `.Codex/prompts/`:
...
- **Slash commands** in `.Codex/commands/`: `/dev`, `/test`, `/deploy`, `/ingest`
- **Rules** in `.Codex/rules/`: `plotlot-backend.md`, `plotlot-frontend.md`, `git-discipline.md`
- **Persona** in `.Codex/prompts/`: soul, spirit, creed, doctrine, mind, principles, personality, user, system

## Rules

1. **Use Codex Sonnet for email generation** as specified in global config.
2. Each project has its own `AGENTS.md` with project-specific instructions.
