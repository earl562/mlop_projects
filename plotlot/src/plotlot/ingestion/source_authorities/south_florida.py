...
def _municode_authority(
    *, state: str, county: str, municipality: str | None,
    ...
    return JurisdictionSourceAuthority(
        ...
        jurisdiction_type=jurisdiction_type, authority_scope=authority_scope,
        provider=Provider.MUNICODE, canonical_url=source_url, source_url=source_url,
        source_title=source_title, official_status=official_status,
        legal_caveat=legal_caveat, metadata_json=metadata or {},
    )
...
def seed_south_florida_authorities() -> list[JurisdictionSourceAuthority]:
    ...
    auths.append(_municode_authority(
        state="FL", county="Miami-Dade", municipality=None,
    ...
    auths.append(JurisdictionSourceAuthority(
