...
def _live_tool_payload(tool_name: str, args: dict[str, object]) -> dict[str, object]:
        ...
        return {
            "profile": {
                ...
                "income_inferred_fields": (
                    ...
                    else ["monthly_rent_per_unit", "vacancy_pct", "operating_expense_pct", "cap_rate"]
                ),
                "income_assumption_source": "user_assumptions" if explicit_income_keys else "market:south_florida",
                "overridden_fields": (
    ...
    if tool_name == "capture_public_listing_comps":
        return {
            ...
            "candidates": [],
            "warnings": ["Optional browser comp capture is not configured for this test."],
        }
...
def _qualifying_land_comparables() -> list[dict[str, object]]:
    return [
        {
...
def _live_tool_result(tool_name: str, run_id: str, args: dict[str, object], source_mode: SourceMode) -> HarnessToolCallResult:
...
async def test_harness_tool_call_api_persists_tool_call_and_appends_events(
    ...
    assert calls_response.json()["tool_calls"][0]["tool_call_id"] == call_response.json()["tool_call_id"]
...
async def test_fixture_backed_harness_endpoints_reject_live_source_mode(
    ...
    for response in (
        ...
        assert response.status_code == 501
        assert response.json()["detail"] == "Only fixture source mode is wired in this harness slice"
...
async def test_deal_analysis_live_run_uses_shared_executor_with_mocked_tools(
    ...
    underwriting_section = next(
        section for section in data["report"]["sections"] if section["section_id"] == "underwriting_summary"
    )
...
async def test_deal_analysis_live_run_omits_comp_signal_without_qualifying_comp_evidence(
    ...
    async def _fake_tool_result(request) -> HarnessToolCallResult:
        payload = _empty_live_comps_payload() if request.tool_name == "find_comparables" else _live_tool_payload(
            request.tool_name,
            dict(request.args),
        )
        return HarnessToolCallResult(
...
async def test_deal_analysis_live_run_uses_ordinance_rules_for_feasibility_defaults(
    ...
    assert underwriting_section["zoning_support_summary"]["authority_confidence"] == "indexed_official_reference"
...
async def test_deal_analysis_live_run_uses_pro_forma_when_noi_inputs_are_missing(
    ...
    assert any(claim["claim_type"] == "max_supportable_land_price" for claim in data["claims"])
    assert not any("NOI valuation skipped" in warning for warning in data["artifacts"].get("warnings", []))
...
async def test_deal_analysis_live_run_skips_pro_forma_when_only_land_comps_exist(
    ...
    async def _fake_tool_result(request) -> HarnessToolCallResult:
        ...
        if request.tool_name == "run_pro_forma":
            raise AssertionError("run_pro_forma should not be called without qualified sold-unit comps")
        return HarnessToolCallResult(
            ok=True,
            tool_call_id=ToolCallId(f"tool_call_{request.tool_name}"),
            tool_name=request.tool_name,
...
async def test_deal_analysis_live_income_path_keeps_cost_assumptions_when_pro_forma_is_skipped(
    ...
    async def _fake_tool_result(request) -> HarnessToolCallResult:
        if request.tool_name == "find_comparables":
            payload = {
                "analysis": {
                    "comparables": _qualifying_land_comparables(),
                    "unit_comparables": [],
...
async def test_deal_analysis_live_run_infers_single_unit_for_vacant_r1_lot(
        ...
        if request.tool_name == "geocode_address":
            ...
            payload = {
                "analysis": {
                    ...
                    "confidence": 0.55,
                    "notes": ["No reliable vacant-land comps within 24 months; using nearby improved single-family sales for exit pricing only."],
                }
            }
    ...
    data = response.json()
    assert data["status"] == "completed"
    assert observed_noi_args["unit_count"] == 1
    assert observed_noi_args["monthly_rent_per_unit"] == pytest.approx(3200.0)
    assert data["artifacts"]["underwriting_mode"]["mode"] == "income_cap_rate"
    assert observed_pro_forma_args == {}
    assert any(call["tool_name"] == "run_residual_land_value" for call in data["tool_calls"])
    assert any("No reliable vacant-land comps" in note for note in data["artifacts"]["comps"]["notes"])
...
async def test_deal_analysis_live_run_uses_staged_vacant_lot_comp_search_and_guidance(
    ...
    async def _fake_tool_result(request) -> HarnessToolCallResult:
        if request.tool_name == "geocode_address":
            ...
            observed_searches.append((int(request.args["months"]), float(request.args["radius_miles"])))
    ...
    assert data["artifacts"]["comp_search_strategy"]["selected_reason"] == "qualified_exit_comp_fallback"
    assert data["artifacts"]["comp_search_strategy"]["attempts"][-1]["radius_miles"] == pytest.approx(6.0)
    ...
    assert data["artifacts"]["comp_search_strategy"]["attempts"][-1]["direct_land_comp_signal"] is False
    ...
    assert data["artifacts"]["comp_search_strategy"]["attempts"][-1]["qualified_exit_comp_signal"] is True
    assert data["artifacts"]["comp_search_strategy"]["attempts"][-1]["selection_reason"] == "qualified_exit_comp_fallback"
    ...
    assert data["artifacts"]["acquisition_guidance"]["market_land_value_low"] == pytest.approx(154391.78)
    assert data["artifacts"]["acquisition_guidance"]["market_land_value_high"] == pytest.approx(154391.78)
    assert data["artifacts"]["acquisition_guidance"]["market_to_residual_gap"] == pytest.approx(249391.78)
    assert data["artifacts"]["comp_search_strategy"]["attempts"][-1]["adv_per_unit"] == pytest.approx(505000.0)
...
async def test_deal_analysis_live_run_prefers_sold_unit_exit_for_vacant_lot_without_explicit_income_inputs(
    ...
    assert "vacant low-density residential lot" in data["artifacts"]["underwriting_mode"]["reason"]
    ...
    assert data["artifacts"]["acquisition_guidance"]["underwriting_mode"] == "sold_unit_exit"
    assert data["artifacts"]["acquisition_guidance"]["exit_comp_signal_available"] is True
...
async def test_deal_analysis_live_run_persists_web_listing_candidates_when_land_signal_is_thin(
    ...
    assert data["artifacts"]["comps"]["web_listing_search"]["browser_capture_provider"] == "browser_use"
    ...
    assert data["artifacts"]["comps"]["browser_listing_candidates"][0]["captured_by"] == "browser_use"
    assert data["artifacts"]["comps"]["web_listing_candidates"][0]["source_domain"] == "www.zillow.com"
    assert data["artifacts"]["comps"]["web_listing_candidates"][0]["classification"] == "likely_vacant_land"
    assert data["artifacts"]["acquisition_guidance"]["recommended_action"] == "insufficient_support"
    assert (
        data["artifacts"]["acquisition_guidance"]["basis"]
        == "comping_underwriting_not_ready"
    )
    assert data["artifacts"]["acquisition_guidance"]["underwriting_mode"] == "blocked_by_comping_gate"
    ...
    assert data["artifacts"]["acquisition_guidance"]["contextual_verified_land_candidate_count"] == 1
    assert data["artifacts"]["acquisition_guidance"]["contextual_land_value_signal"] == pytest.approx(151575.0)
    assert data["artifacts"]["acquisition_guidance"]["contextual_market_land_value_low"] == pytest.approx(151575.0)
    assert data["artifacts"]["acquisition_guidance"]["contextual_market_land_value_high"] == pytest.approx(151575.0)
    assert data["artifacts"]["acquisition_guidance"]["county_reconciled_land_candidate_count"] == 0
    assert data["artifacts"]["acquisition_guidance"]["county_reconciled_land_value_signal"] == pytest.approx(0.0)
    assert data["artifacts"]["acquisition_guidance"]["county_reconciled_market_land_value_low"] == pytest.approx(0.0)
    assert data["artifacts"]["acquisition_guidance"]["county_reconciled_market_land_value_high"] == pytest.approx(0.0)
    assert data["artifacts"]["acquisition_guidance"]["land_signal_source"] == "contextual_public_listing"
    assert data["artifacts"]["acquisition_guidance"]["land_signal_strength"] == "contextual"
    assert data["artifacts"]["acquisition_guidance"]["market_signal_verification_status"] == "contextual_verified"
    ...
    assert data["artifacts"]["comp_search_strategy"]["land_signal_tier"] == "contextual_public_listing"
    assert data["artifacts"]["comp_search_strategy"]["county_reconciled_candidate_count"] == 0
    assert data["artifacts"]["comp_search_strategy"]["public_listing_signal_tier"] == "contextual_verified"
    assert data["artifacts"]["comp_search_strategy"]["public_listing_land_comp_count"] == 1
    assert data["artifacts"]["comp_search_strategy"]["best_public_listing_fit_score"] == pytest.approx(0.891)
    assert data["artifacts"]["comp_search_strategy"]["best_public_listing_lot_size_variance_ratio"] == pytest.approx(
        0.109
    )
    assert data["artifacts"]["comp_search_strategy"]["best_public_listing_sale_date"] == "2026-04-21"
    assert data["artifacts"]["comp_search_strategy"]["public_listing_market_scope"] == "cross_zip_same_municipality"
    assert data["artifacts"]["comp_search_strategy"]["public_listing_recency_tier"] == "recent_6m"
    assert data["artifacts"]["comp_search_strategy"]["best_public_listing_parse_confidence"] == pytest.approx(1.0)
    assert data["artifacts"]["comp_search_strategy"]["public_listing_domains"] == ["www.zillow.com"]
    assert data["artifacts"]["comps"]["public_listing_land_comparables"][0]["verification_status"] == "contextual_verified"
    assert data["artifacts"]["comps"]["public_listing_land_comparables"][0]["source_domain"] == "www.zillow.com"
    public_listing_section = next(
        section for section in data["report"]["sections"] if section["section_id"] == "public_listing_comps"
    )
    ...
    assert data["artifacts"]["contextual_land_listing_verification"]["verified_candidate_count"] == 1
    ...
    assert comp_support_summary["comping_underwriting_status"] == "blocked_pending_county_reconciliation"
    ...
    web_evidence = [
        item for item in data["evidence_items"] if item["provider"] == "exa_web_search"
    ]
    ...
    assert {
        item["structured_payload"]["address_hint"]
    ...
    county_reconciled_evidence = [
        item for item in data["evidence_items"] if item["provider"] == "county_reconciled_public_listing"
    ]
    ...
    assert comp_only_workflow["trust_gates"]["underwriting_status"] == "blocked_pending_county_reconciliation"
...
async def test_deal_analysis_live_run_prefers_direct_land_comps_without_contextual_listing_fetch(
    ...
    assert data["artifacts"]["acquisition_guidance"]["market_signal_verification_status"] == "direct_verified"
    ...
    assert data["artifacts"]["comp_search_strategy"]["best_direct_land_comp_fit_score"] == pytest.approx(0.99)
    assert data["artifacts"]["comp_search_strategy"]["best_direct_land_comp_lot_size_variance_ratio"] == pytest.approx(
        0.01
    )
    assert data["artifacts"]["comp_search_strategy"]["best_direct_land_comp_qualification_score"] == pytest.approx(
        0.9
    )
    underwriting_section = next(
        section for section in data["report"]["sections"] if section["section_id"] == "underwriting_summary"
    ...
    assert underwriting_section["comp_support_summary"]["land_support_source"] == "direct_land_comps"
    assert underwriting_section["comp_support_summary"]["land_support_fit_score"] == pytest.approx(0.99)
    assert underwriting_section["comp_support_summary"]["exit_support_fit_score"] == pytest.approx(1.0)
    assert underwriting_section["comp_support_summary"]["exit_support_market_scope"] == "subject_municipality"
...
async def test_deal_analysis_live_run_keeps_contextual_land_signal_preliminary_when_county_reconciliation_fails(
    ...
    assert underwriting_section["comp_support_summary"]["land_support_source"] == "contextual_public_listing"
    assert underwriting_section["comp_support_summary"]["land_support_fit_score"] == pytest.approx(0.891)
    assert underwriting_section["comp_support_summary"]["land_support_market_scope"] == "cross_zip_same_municipality"
    ...
    assert underwriting_section["comp_support_summary"]["land_support_parse_confidence"] == pytest.approx(1.0)
...
async def test_deal_analysis_live_run_uses_browser_captured_county_reconciled_land_signal_for_underwriting(
    ...
    assert data["artifacts"]["comping_workflow"]["trust_gates"]["underwriting_status"] == "available_to_underwriting"
    assert data["artifacts"]["comping_workflow"]["county_reconciled_public_listing_comps"][0]["address"] == (
        "17605 NW 19th Avenue, Miami Gardens, FL 33056"
    )
    reconciliation = data["artifacts"]["contextual_land_listing_reconciliation"]
...
async def test_deal_analysis_live_run_keeps_selected_attempt_in_sync_with_saved_comp_payload(
    ...
    assert data["artifacts"]["comp_search_strategy"]["best_exit_comp_fit_score"] == pytest.approx(1.0)
    assert data["artifacts"]["comp_search_strategy"]["best_exit_comp_price_variance_ratio"] == pytest.approx(0.0)
    assert data["artifacts"]["comp_search_strategy"]["best_exit_comp_qualification_score"] == pytest.approx(0.9)
    ...
    assert (
        ...
        == "blocked_missing_land_comp_support"
    )
    assert data["artifacts"]["underwriting_mode"]["mode"] == "blocked_by_comping_gate"
    assert data["artifacts"]["acquisition_guidance"]["recommended_action"] == "insufficient_support"
    assert (
    ...
    assert data["artifacts"]["acquisition_guidance"]["recommended_offer"] == pytest.approx(0.0)
    assert data["artifacts"]["acquisition_guidance"]["max_supportable_land_price"] == pytest.approx(
        0.0
    )
    assert data["artifacts"]["acquisition_guidance"]["land_comp_signal_available"] is False
    assert data["artifacts"]["acquisition_guidance"]["exit_comp_signal_available"] is True
...
async def test_deal_analysis_live_run_derives_feasibility_from_verified_dimensional_standard(
    ...
    feasibility_stage = next(stage for stage in data["pipeline_stages"] if stage["key"] == "feasibility")
...
async def test_deal_analysis_live_run_accepts_manual_single_family_dimensional_inputs(
    ...
    assert data["artifacts"]["gis_site_context"]["controlling_zoning_jurisdiction"] == "Miami Gardens"
    ...
    assert data["artifacts"]["manual_dimensional_standards"]["max_lot_coverage_pct"] == pytest.approx(40.0)
    assert data["artifacts"]["manual_dimensional_standards"]["min_lot_frontage_ft"] == pytest.approx(75.0)
    assert data["artifacts"]["manual_dimensional_standards"]["rear_setback_ft"] == pytest.approx(25.0)
    ...
    assert data["report"]["sections"][1]["feasibility"]["result"]["max_gross_buildable_sf"] == pytest.approx(
        4042.0
    )
...
async def test_deal_analysis_live_run_uses_manual_land_and_exit_comps(
    ...
    assert data["artifacts"]["acquisition_guidance"]["market_land_value_low"] == pytest.approx(147785.62, abs=0.01)
    assert data["artifacts"]["acquisition_guidance"]["market_land_value_high"] == pytest.approx(150311.88, abs=0.01)
    ...
    assert all(
        "rear setback" not in warning.lower()
        ...
        if request.tool_name == "geocode_address":
            ...
            payload = {
                "analysis": {
                    ...
                    "confidence": 0.55,
                    "notes": ["Land comps suggest the seller may expect more than a residual buyer can support."],
                }
            }
        elif request.tool_name == "run_residual_land_value":
            payload = {
...
async def test_deal_analysis_live_run_falls_back_to_verified_local_ordinance_context(
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["artifacts"]["ordinance_search"]["fallback_source"] == "verified_dimensional_standard"
    assert data["artifacts"]["ordinance_search"]["authority_confidence"] == "indexed_official_reference"
    ...
    assert underwriting_section["zoning_support_summary"]["ordinance_source"] == "verified_dimensional_standard"
    assert underwriting_section["zoning_support_summary"]["authority_confidence"] == "indexed_official_reference"
    ...
    zoning_stage = next(stage for stage in data["pipeline_stages"] if stage["key"] == "zoning_evidence")
...
async def test_deal_analysis_live_run_falls_back_to_staged_local_ordinance_context(
    ...
    assert underwriting_section["zoning_support_summary"]["ordinance_source"] == "staged_dimensional_standard"
    assert underwriting_section["zoning_support_summary"]["authority_confidence"] == "staged_preliminary"
    assert underwriting_section["zoning_support_summary"]["requires_official_verification"] is True
    zoning_stage = next(stage for stage in data["pipeline_stages"] if stage["key"] == "zoning_evidence")
    assert zoning_stage["status"] == "warning"
