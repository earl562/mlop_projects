...
Direction A's selected reference is `../direction-a/reference-direction-a-v3.png`. It is bound to canonical prompt `direction-a/v1.1.0` and exactly one fresh built-in `image_gen.imagegen` call for this truth correction by `../direction-a/imagegen.metadata.json`. The rail visibly abstains from maximum units and purchase ceiling while parking is not hash-bound, matching `STATE_MATRIX.md` lines 42–50. Direction A v1 and rejected v2 remain historical references only. This correction does not change the deterministic scores or selection.
