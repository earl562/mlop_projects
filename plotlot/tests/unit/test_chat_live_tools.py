...
@pytest.mark.asyncio
async def test_execute_tool_run_deal_analysis_stores_harness_analysis_in_session(monkeypatch) -> None:
...
async def test_execute_tool_routes_new_live_tools():
    ...
    with (
        patch(
            ...
            new=AsyncMock(
                side_effect=[
                    json.dumps({"status": "success", "payload": {"status": "success", "kind": "open_data"}}),
                    json.dumps({"status": "success", "payload": {"status": "success", "kind": "municode"}}),
                ]
            ),
        ),
    ):
