...
from plotlot.harness.report_inspection import (
...
def _stale_evidence_ids(evidence_items: Sequence[EvidenceItem]) -> list[str]:
    ...
    return sorted(
        str(item.evidence_id) for item in evidence_items if item.freshness_status in stale_statuses
    )
...
def _mock_or_fixture_blockers(report: Report, evidence_items: Sequence[EvidenceItem]) -> list[str]:
    ...
    for item in evidence_items:
        if item.source_mode not in {
            SourceMode.FIXTURE,
            SourceMode.MOCK,
        } and item.freshness_status not in {
