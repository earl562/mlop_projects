...
from plotlot.domain.types import EvidenceConfidence, EvidenceItem, SourceType
...
from plotlot.harness.fixture_site_data import fixture_comp_analysis, fixture_site_profile_for_address
...
def default_tool_handlers() -> dict[str, ToolHandler]:
    return {
        ...
        "search_zoning_ordinance": _delegate_runtime_handler(runtime_handle_search_zoning_ordinance),
...
async def _handle_find_comparables(request: HarnessToolCallRequest) -> JsonObject:
    ...
    if request.source_mode is SourceMode.FIXTURE and _is_fixture_comps_subject(args):
        analysis = fixture_comp_analysis(fixture_site_profile_for_address(args.address or args.county))
    else:
    ...
    run_request = FixtureDealRunRequest.model_validate(
        {
            ...
            "analysis_type": request.args.get("analysis_type") or request.args.get("analysisType") or "acquisition_memo",
            "source_mode": request.source_mode,
...
def _delegate_runtime_handler(
    handler: Callable[[dict[str, object], object], Awaitable[JsonObject]],
) -> ToolHandler:
...
async def _enrich_comparables_with_web_candidates(
    ...
    *,
    ...
    if not settings.exa_api_key:
        ...
        return payload
    merged_evidence = list(payload.get("evidence")) if isinstance(payload.get("evidence"), list) else []
    for query_plan in build_comparable_listing_queries(subject_payload):
        ...
        land_candidate_count = len(
            [candidate for candidate in query_candidates if candidate.get("classification") == "likely_vacant_land"]
        )
        improved_candidate_count = len(
            [candidate for candidate in query_candidates if candidate.get("classification") == "likely_improved_sale"]
        )
        query_attempts.append(
            {
                "query": query_plan.query,
...
def _web_listing_search_payload(
    *,
    ...
    return {
        ...
        "selected_purpose": selected_attempt.get("purpose") if isinstance(selected_attempt, dict) else "",
        "status": status or (selected_attempt.get("status") if isinstance(selected_attempt, dict) else ""),
        ...
        "selected_search_window_months": (
            selected_attempt.get("search_window_months") if isinstance(selected_attempt, dict) else None
        ),
