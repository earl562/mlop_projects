...
def _result(
    tool_name: str, run_id: str, args: dict[str, object], payload: dict[str, object]
...
def test_cli_run_live_auto_comp_path_surfaces_exit_comp_supportability(
    ...
    assert (
        payload["artifacts"]["comp_search_strategy"]["selected_reason"]
        == "qualified_exit_comp_fallback"
    )
    assert (
        payload["artifacts"]["comp_search_strategy"]["attempts"][-1]["qualified_exit_comp_signal"]
        is True
    )
    assert (
        payload["artifacts"]["comp_search_strategy"]["attempts"][-1]["strong_unit_comp_count"] == 2
    )
    assert (
        payload["artifacts"]["acquisition_guidance"]["recommended_action"] == "insufficient_support"
    )
