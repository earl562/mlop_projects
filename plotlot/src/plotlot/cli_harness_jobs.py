...
def jobs_command(args: list[str]) -> int:
    ...
    try:
        match args[0]:
            ...
            case "list":
                print(json.dumps({"jobs": [job.model_dump(mode="json") for job in queue.list_jobs()]}))
                return 0
            case "show" if len(args) >= 2:
                print(json.dumps(queue.get_job(JobId(args[1])).model_dump(mode="json")))
                return 0
            case "events" if len(args) >= 2:
                events = [event.model_dump(mode="json") for event in queue.get_events(JobId(args[1]))]
...
def _jobs_create(args: list[str], queue: LocalHarnessJobQueue) -> int:
    ...
    if max_attempts is None:
        ...
        return 2
    analysis_type = (option_value(options, "--analysis-type") or "acquisition_memo").replace("-", "_")
