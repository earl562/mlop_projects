...
from plotlot.harness.listing_comp_support import contextual_fit_score, lot_size_variance_ratio, parse_iso_date
...
async def run_fixture_deal_analysis_async(request: FixtureDealRunRequest) -> FixtureDealRunResult:
    ...
    if isinstance(selected_gis, dict):
        ...
        evidence_items.append(_gis_source_evidence(run_id=run_id, profile=profile, source_payload=selected_gis))
    ...
    match normalized_analysis:
        ...
        case "comparable_comping":
            ...
            evidence_items.extend(_comp_evidence(run_id=run_id, profile=profile, comps_payload=comps_payload))
        ...
        case "acquisition_memo" | "development_underwriting" | "lender_package":
            ...
            feasibility_inputs = _feasibility_inputs(profile=profile, assumptions=request.assumptions)
...
async def _run_live_deal_analysis_async(request: FixtureDealRunRequest) -> FixtureDealRunResult:
    ...
    if ordinance_payload is not None:
        ...
        _append_warning(artifacts, "No ordinance search result was available; zoning remains preliminary.")
    ...
    if normalized_analysis == "comparable_comping":
        ...
        if contextual_land_verification:
            ...
            contextual_land_reconciliation = await _reconcile_contextual_land_candidates_with_county_records(
                run_id=run_id,
                request=request,
                context=tool_context,
                events=events,
            ...
            if contextual_land_reconciliation:
                ...
                comps_payload["contextual_land_listing_reconciliation"] = contextual_land_reconciliation
        ...
        artifacts["underwriting_mode"] = _underwriting_mode_payload(
            ...
            pricing_source="manual_comps" if bool(comps_payload.get("manual_comp_override")) else "auto_comps",
        )
        ...
        if (
            underwriting_strategy.mode == "income_cap_rate"
            and bool(cost_assumptions.get("requires_income_assumption_verification"))
        ):
            _append_warning(
                artifacts,
            ...
            if noi_resolution.inputs is not None:
                ...
                if residual_inputs is not None:
                    ...
                    artifacts["underwriting_mode"] = _underwriting_mode_payload(
                        ...
                        source_artifacts=["noi_valuation", "residual_land_value", "cost_assumptions"],
                        pricing_source=pricing_source,
                    )
                else:
                    _append_warning(
    ...
    *,
    ...
    if (
        ...
        if _has_supported_relaxed_land_comp_signal(merged_payload):
            ...
            if selected_attempt_index is not None and selected_attempt_index < len(attempts):
                attempts[selected_attempt_index]["selection_reason"] = "supported_relaxed_land_signal"
    ...
    if selected_attempt_index is not None:
        for index, attempt in enumerate(attempts):
            ...
            elif index == selected_attempt_index and attempt["selection_reason"] == "qualified_exit_comp_fallback_candidate":
...
async def _verify_contextual_web_land_candidates(
    *,
    ...
    verification = build_contextual_land_listing_verification(
        candidates=[candidate for candidate in web_listing_candidates if isinstance(candidate, dict)],
    ...
    if (
        ...
        and int(verification.get("review_candidate_count") or 0) <= 0
    ):
        return None
    return verification
...
async def _reconcile_contextual_land_candidates_with_county_records(
    *,
    ...
    if not isinstance(verified_candidates, list):
        return None
    review_candidates = contextual_land_verification.get("review_candidates")
    if not isinstance(review_candidates, list):
        review_candidates = []
    reconciliation_candidates = [*verified_candidates, *review_candidates]

    reconciled_candidates: list[JsonObject] = []
    rejected_candidates: list[JsonObject] = []
    subject_lot_area_sf = float(property_payload.get("lot_size_sqft") or 0.0)
    for raw_candidate in _county_reconciliation_candidates(reconciliation_candidates):
        if not isinstance(raw_candidate, dict):
            continue
        candidate_address = str(raw_candidate.get("address_hint") or raw_candidate.get("title") or "").strip()
    ...
    if subject_lot_area_sf <= 0:
        return _empty_county_reconciliation_result(
            verified_candidates=reconciliation_candidates,
...
def _county_reconciliation_rejection(*, raw_candidate: JsonObject, reason: str) -> JsonObject:
    return {
        "address": str(raw_candidate.get("address_hint") or raw_candidate.get("title") or "").strip(),
...
def _listing_candidate_matches_county_record(candidate: CountyReconciliationCandidate) -> bool:
    ...
    if not _sale_dates_align(candidate.listed_sale_date, candidate.county_sale_date):
        return False
    sale_price_ratio = abs(candidate.county_sale_price - candidate.listed_sale_price) / candidate.listed_sale_price
    lot_size_ratio = abs(candidate.county_lot_size_sqft - candidate.listed_lot_size_sqft) / candidate.listed_lot_size_sqft
...
def _county_reconciliation_candidates(
    ...
    ranked_candidates = [candidate for candidate in verified_candidates if isinstance(candidate, dict)]
    ...
    for candidate in ranked_candidates:
        ...
        if existing is None or _county_reconciliation_sort_key(candidate) < _county_reconciliation_sort_key(existing):
...
def _county_reconciliation_dedupe_key(candidate: JsonObject) -> str:
    address_hint = str(candidate.get("address_hint") or candidate.get("title") or "").strip().casefold()
    return address_hint


def _county_reconciled_pricing_candidates(reconciled_candidates: list[JsonObject]) -> list[JsonObject]:
...
def _county_reconciled_pricing_scope(reconciled_candidates: list[JsonObject]) -> str:
    if not reconciled_candidates:
        return "unknown"
    if all(candidate.get("zip_match") is True for candidate in reconciled_candidates if isinstance(candidate, dict)):
        return "subject_zip"
    if all(
        candidate.get("municipality_match") is True for candidate in reconciled_candidates if isinstance(candidate, dict)
    ):
    ...
    *,
    ...
    if manual_land_comps:
        land_values = [
            ...
            if float(comp.get("lot_size_sqft") or 0.0) > 0 and float(comp.get("sale_price") or 0.0) > 0
        ]
    ...
    if manual_exit_comps:
        unit_values = [
            ...
            if isinstance(comp.get("price_per_unit"), int | float) and float(comp["price_per_unit"]) > 0
        ]
        ...
        if unit_values:
            ...
            merged_payload["adv_per_unit_low"] = round(_percentile_value(unit_values_sorted, 0.25), 2)
            merged_payload["adv_per_unit_high"] = round(_percentile_value(unit_values_sorted, 0.75), 2)
...
def _property_evidence(
    *,
    ...
    evidence_municipality = str(
        (property_payload or {}).get("municipality") or property_record.municipality or profile.municipality
    )
    evidence_county = str(
        (property_payload or {}).get("county") or property_record.county or profile.county
    )
    evidence_zoning_code = str(
        (property_payload or {}).get("zoning_code") or property_record.zoning_code or profile.zoning_code
    )
    evidence_lot_size_sqft = float(
        (property_payload or {}).get("lot_size_sqft") or property_record.lot_size_sqft or profile.lot_size_sqft
    )
    ...
    *,
    ...
    for comp_type in ("comparables", "unit_comparables"):
        ...
        for index, raw_item in enumerate(raw_items):
            ...
            metadata: JsonObject = {"fixture_profile": profile.key, "land_comp_quality": land_comp_quality}
            normalized_text = (
...
def _feasibility_inputs(*, profile: FixtureSiteProfile, assumptions: JsonObject) -> JsonObject:
    inputs: JsonObject = {
        ...
        "avg_unit_size_sf": _float_assumption(assumptions, "avgUnitSizeSf", profile.avg_unit_size_sf),
...
def _noi_inputs(*, profile: FixtureSiteProfile, assumptions: JsonObject) -> JsonObject:
    ...
    monthly_rent = _float_assumption(assumptions, "monthlyRentPerUnit", profile.monthly_rent_per_unit)
    ...
    *,
    ...
    return [
        Claim(
            ...
            source_url=str(public_listing_search.get("query") or "https://plotlot.local/market-comps"),
            next_verification_step=(
...
def _comp_support_summary(artifacts: JsonObject) -> JsonObject:
    ...
    land_support_source, land_support_fit_score, land_support_quality_score = _land_support_snapshot(
        strategy=strategy,
    ...
    return {
        ...
        "reason": reason,
        "comping_underwriting_status": comping_underwriting_status,
        "comping_underwriting_blocker": _comping_underwriting_blocker(
        ...
        "land_support_parse_confidence": float(strategy.get("best_public_listing_parse_confidence") or 0.0),
...
def _zoning_support_summary(artifacts: JsonObject) -> JsonObject:
    ...
    fallback_source = str(ordinance_search.get("fallback_source") or ordinance_rules.get("source") or "none").strip()
...
def _public_listing_market_scope(public_listing_land_comparables: list[JsonObject]) -> str:
    ...
    if zip_match_count == len(public_listing_land_comparables):
        return "subject_zip"
    if matched_count == len(public_listing_land_comparables) and zip_known_count > 0 and zip_match_count == 0:
...
def _public_listing_micro_market_confidence(public_listing_land_comparables: list[JsonObject]) -> str:
    if not public_listing_land_comparables:
        return "none"
    zip_match_count = sum(
        1
    ...
    *,
    ...
    if recommended_action == "insufficient_support":
        if exit_support_fit_score >= 0.8:
            return "exit_only"
        return "weak"
    if land_signal_tier == "direct_land_comps" and exit_support_fit_score >= 0.8 and land_support_fit_score >= 0.8:
        return "balanced"
...
def _land_comp_cluster_key(raw_item: JsonObject) -> tuple[str, int, int] | None:
    ...
    if not sale_date or not isinstance(sale_price, int | float) or not isinstance(lot_size_sqft, int | float):
        return None
    return (
        sale_date,
...
def _land_comp_quality_summary(comps_payload: JsonObject) -> JsonObject:
    ...
    best_fit_score, best_fit_variance_ratio, best_fit_qualification_score = _best_land_comp_fit_metrics(
        comps_payload=comps_payload
    )
...
def _best_unit_comp_fit_metrics(comps_payload: JsonObject) -> tuple[float, float, float]:
    ...
    if not isinstance(unit_comparables, list) or not isinstance(adv_per_unit, int | float) or float(adv_per_unit) <= 0:
        return 0.0, 0.0, 0.0
...
def _build_comp_search_strategy_summary(
    *,
    ...
    land_comp_quality = _land_comp_quality_summary(comps_payload)
    unit_comp_quality = _unit_comp_quality_summary(comps_payload)
    selected_attempt = next((attempt for attempt in attempts if bool(attempt.get("selected"))), None)
    ...
    public_listing_land_comparables = _required_list(comps_payload, "public_listing_land_comparables")
    unit_comparables = _required_list(comps_payload, "unit_comparables")
    ...
    if public_listing_land_comparables:
        ...
        public_listing_signal_tier = str(first_public_listing.get("verification_status") or "contextual_verified")
    ...
    return {
        "attempts": attempts,
        "selected_months": selected_attempt.get("months") if isinstance(selected_attempt, dict) else None,
        "selected_reason": (
        ...
        "best_exit_comp_distance_miles": float(exit_support_snapshot.get("exit_support_distance_miles") or 0.0),
        "best_exit_comp_sale_date": str(exit_support_snapshot.get("exit_support_sale_date") or ""),
        "exit_support_market_scope": str(exit_support_snapshot.get("exit_support_market_scope") or "unknown"),
        "exit_micro_market_confidence": exit_micro_market_confidence,
        "exit_comp_recency_tier": str(exit_support_snapshot.get("exit_support_recency_tier") or "unknown"),
        "public_listing_signal_tier": public_listing_signal_tier,
        ...
        "public_listing_market_scope": _public_listing_market_scope(public_listing_land_comparables),
...
def _manual_comp_entries(*, raw_value: object, role: str) -> list[JsonObject]:
    ...
    for raw_item in raw_value:
        ...
        normalized: JsonObject = {
            ...
            "lot_size_sqft": round(float(lot_size_sqft), 2) if isinstance(lot_size_sqft, int | float) else 0.0,
            "zoning_code": str(raw_item.get("zoningCode") or "").strip(),
            "distance_miles": round(float(raw_item.get("distanceMiles")), 3)
            if isinstance(raw_item.get("distanceMiles"), int | float)
            else 0.0,
            "provider": "user_provided_comp",
            "source_url": str(raw_item.get("sourceUrl") or "").strip() or "https://plotlot.local/user-input/comps",
            "user_supplied": True,
            "comp_role": role,
        }
        ...
        if role == "exit":
            unit_count = float(units) if isinstance(units, int | float) and float(units) > 0 else 1.0
    ...
    *,
    ...
    if _uses_miami21_special_authority(municipality):
        ...
        if _required_list(web_call.payload, "results"):
            if fallback_payload is not None and _live_ordinance_rules_payload(web_call.payload) is None:
                return {
                    **web_call.payload,
...
async def _fallback_live_ordinance_payload(
    *,
    ...
    fallback_source = "verified_dimensional_standard" if is_verified else "staged_dimensional_standard"
    ...
    summary_parts = [
        ...
        f"front setback {standard.setback_front_ft} ft" if standard.setback_front_ft is not None else None,
        f"side setback {standard.setback_side_ft} ft" if standard.setback_side_ft is not None else None,
        f"rear setback {standard.setback_rear_ft} ft" if standard.setback_rear_ft is not None else None,
    ...
    *,
    ...
    return EvidenceItem(
        ...
        structured_payload={
            ...
            "controlling_zoning_jurisdiction": gis_site_context.get("controlling_zoning_jurisdiction"),
...
def _live_ordinance_evidence(
    *,
    ...
    county = _county_name(str(property_payload.get("county") or "Unknown"))
    municipality = str(property_payload.get("municipality") or "") or None
    title = str(first.get("title") or first.get("heading") or first.get("section") or "Ordinance section")
    ...
    *,
    ...
    if isinstance(web_listing_search, dict) and isinstance(web_listing_candidates, list):
        for index, raw_result in enumerate(web_listing_candidates):
            ...
            normalized_text = str(raw_result.get("description") or raw_result.get("address_hint") or title)
            ...
            for index, raw_result in enumerate(verified_candidates):
                ...
                items.append(
                    EvidenceItem(
                        ...
                        applicability=ApplicabilityStatus.CONTEXTUAL,
                        normalized_text=normalized_text,
                        structured_payload={"comp_type": "contextual_verified_land_listing", **raw_result},
                        confidence=0.58,
                        source_mode=source_mode,
                        metadata={
                            "live": True,
    ...
    if isinstance(contextual_land_reconciliation, dict):
        ...
        if isinstance(reconciled_candidates, list):
            for index, raw_result in enumerate(reconciled_candidates):
                if not isinstance(raw_result, dict):
                    continue
                source_url = str(raw_result.get("url") or "https://plotlot.local/county-reconciled-listing")
                title = str(raw_result.get("title") or raw_result.get("address_hint") or source_url)
                normalized_text = (
                ...
                items.append(
                    EvidenceItem(
                        evidence_id=EvidenceId(f"ev_{run_id}_county_reconciled_land_listing_{index + 1}"),
                        run_id=run_id,
                        source_type=EvidenceSourceType.MARKET_COMP,
                        source_name=title,
                        source_url=source_url,
                        ...
                        applicability=ApplicabilityStatus.CONTEXTUAL,
                        normalized_text=normalized_text,
                        structured_payload={"comp_type": "county_reconciled_land_listing", **raw_result},
                        confidence=0.66,
                        source_mode=source_mode,
                        metadata={
                            "live": True,
...
async def _live_feasibility_inputs(
    *,
    ...
    if (
        not isinstance(max_far, int | float)
        or not isinstance(max_units, int | float)
    ) and lot_area > 0:
        ...
        if standard is not None:
            ...
            if standard.max_density_units_per_acre is not None and standard.max_density_units_per_acre > 0:
            ...
            preliminary = _lookup_preliminary_live_dimensional_standard(property_payload=property_payload)
            if preliminary is not None:
                standard_defaults = preliminary
                max_far = preliminary.far
                if preliminary.max_density_units_per_acre is not None and preliminary.max_density_units_per_acre > 0:
                    max_units = floor((lot_area / 43_560.0) * preliminary.max_density_units_per_acre)
                elif preliminary.min_lot_area_sqft is not None and preliminary.min_lot_area_sqft > 0:
    ...
    if (
        ...
        and (
            isinstance(max_far, int | float)
            or isinstance(max_units, int | float)
        )
    ):
        warning = (
    ...
    if (
        ...
        and (min_lot_frontage_ft := _optional_float_assumption(assumptions, "minLotFrontageFt")) is not None
...
def _lookup_preliminary_live_dimensional_standard(
    *,
    ...
    municipality = str(property_payload.get("municipality") or "").strip()
    zoning_code = str(property_payload.get("ordinance_district_code") or property_payload.get("zoning_code") or "").strip()
    ...
    if reconciled_candidates:
            ...
            public_listing_land_comparables.append(
                {
                    ...
                    "sale_date": raw_candidate.get("county_sale_date") or raw_candidate.get("sale_date"),
        ...
        for raw_candidate in verified_candidates:
            ...
            public_listing_land_comparables.append(
                {
                    "address": str(
                        raw_candidate.get("address_hint")
                        or raw_candidate.get("title")
                        or ""
                    ).strip(),
...
def _ordinance_payload_matches_requested_context(
    *,
    ...
    for result in results:
        ...
        zone_code_list = [str(item).upper() for item in zone_codes] if isinstance(zone_codes, list) else []
        if _uses_miami21_special_authority(municipality):
            municipality_matches = (
                not jurisdiction
                or ("miami" in jurisdiction and "unincorporated" not in jurisdiction)
            )
        else:
            municipality_matches = bool(jurisdiction) and municipality_key in jurisdiction
        zoning_matches = not zoning_key or zoning_key in zone_code_list or zoning_key in title or zoning_key in text
...
def _live_noi_inputs(
    *,
    ...
    if not all(
        ...
        for value in (monthly_rent, vacancy_pct, operating_expense_pct, cap_rate)
    ):
...
def _merge_live_land_comp_payloads(
    *,
    ...
    notes = _string_list(merged_payload.get("notes"))
    if "Merged land comps across search radii for stronger county-backed land coverage." not in notes:
        notes.append("Merged land comps across search radii for stronger county-backed land coverage.")
    merged_payload["notes"] = notes
...
def _infer_vacant_single_family_unit_count(property_payload: JsonObject) -> int | None:
    ...
    land_use_description = str(property_payload.get("land_use_description") or "").upper()
    if not any(zoning.startswith(prefix) for prefix in ("RS", "R-1", "R1", "RE", "RH", "SF", "SFR")):
        return None
...
def _has_explicit_income_underwriting_inputs(assumptions: JsonObject) -> bool:
    ...
    return all(isinstance(assumptions.get(field_name), int | float) for field_name in required_fields)
...
def _live_zoning_claims(
    *,
    ...
    zoning_source_url = str(property_payload.get("zoning_layer_url") or "").strip() or "https://plotlot.local/zoning-record"
    claims = [
        Claim(
...
def _build_live_acquisition_guidance(
    *,
    ...
    land_value_high = _market_land_value_bound(comps_payload, "estimated_land_value_high", land_value)
    ...
    guidance_land_value_low = (
        effective_land_value_low
        if effective_land_value_low is not None
        else land_value_low
    )
    guidance_land_value_high = (
        effective_land_value_high
        if effective_land_value_high is not None
        else land_value_high
    )
        ...
        if not has_land_signal and not has_manual_land_comp_override:
            ...
            if land_signal_strength == "county_reconciled":
                capped_offer_low = _capped_offer_bound(
                    market_bound=effective_land_value_low,
                    max_offer=float(max_offer),
                )
                capped_offer_high = _capped_offer_bound(
                    market_bound=effective_land_value_high,
                    max_offer=float(max_offer),
                )
                return {
                    ...
                    **_guidance_meta(
                    ...
                    "max_supportable_land_price": float(max_offer),
                    "land_value_signal": float(effective_land_value) if effective_land_value is not None else 0.0,
                    "market_land_value_low": guidance_land_value_low,
                    "market_land_value_high": guidance_land_value_high,
                    "adv_per_unit": float(adv_per_unit) if isinstance(adv_per_unit, int | float) else 0.0,
                    "underwriting_mode": mode,
...
def _has_qualified_live_unit_comp_signal(comps_payload: JsonObject) -> bool:
    ...
    has_strong_exit_support = strong_unit_scores >= 2 or (
        very_strong_unit_scores >= 1 and isinstance(unit_comparables, list) and len(unit_comparables) >= 1
    )
