...
@pytest.fixture(autouse=True)
def _deterministic_llm_and_cache_isolation():
    """Keep the suite independent of the developer's .env and of live caches.

    Two real failures motivated this. (1) `PLOTLOT_FAST_EXTRACTION=1` in a local
    .env made `call_llm_fast` route to Groq, so tests that patch
    `plotlot.retrieval.llm.call_llm` were bypassed and hit the network — the
    extraction pipeline then produced no params and unit counts silently came
    back 0. (2) The assessor lookup's durable cache reads the configured
    database, so a unit test asserting an assessor MISS was served the real
    cached lot (7,710 sqft) instead. Both make results depend on machine state,
    which is exactly what a unit suite must not do.
    """
    ...
    with (
        patch.object(settings, "fast_extraction_enabled", False),
        patch(
