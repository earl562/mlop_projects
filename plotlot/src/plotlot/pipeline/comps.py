...
def _is_vacant_single_family_subject(subject: PropertyRecord) -> bool:
    ...
    is_low_density = any(zoning.startswith(prefix) for prefix in ("RS", "R-1", "R1", "RE", "RH", "SF", "SFR"))
...
def _select_vacant_single_family_unit_comps(
    ...
    filtered = [
        ...
        if comp.lot_size_sqft > 0 and (comp.price_per_unit or 0.0) > 0
    ]
    if not filtered:
        return [], None
...
def _prioritize_recent_new_build_unit_comps(
    ...
    recent_builds = [
        comp
        for comp in unit_comps
        if _is_recent_new_build_year(comp.adjustments.get("year_built"))
    ]
...
async def _enrich_broward_sales_features(
    ...
    folios = sorted({_broward_comp_folio(feature.get("attributes", {})) for feature in features if _broward_comp_folio(feature.get("attributes", {}))})
    if not folios:
        return features
            ...
            for offset in range(0, limit, page_size):
                ...
                for attempt in range(1, COMPS_QUERY_MAX_ATTEMPTS + 1):
                    try:
                        ...
                        if _is_transient_comps_query_error(exc) and attempt < COMPS_QUERY_MAX_ATTEMPTS:
...
async def find_comparables(
    ...
    if not land_comps and relaxed_land_comps:
        if vacant_single_family_subject:
            ...
            if promoted_land_comps:
                ...
                land_recent_flags = [
                    is_recent
                    for comp, is_recent in zip(relaxed_land_comps, relaxed_land_recent_flags, strict=False)
    ...
    if vacant_single_family_subject and not land_comps and len(unit_comps) >= 2 and result.adv_per_unit > 0:
