...
async def analyze_stream(request: AnalyzeRequest):
    ...
    async def event_generator():
        try:
            ...
            prop_record = None
            backend_error: tuple[str, str] | None = None
            for _tick in range(4):  # 4 × 10s = 40s max
                ...
                if done:
                    try:
                        ...
                        detail, err_type = _describe_pipeline_error(e)
                        if err_type == "backend_unavailable":
                            backend_error = (detail, err_type)
                    break
