...
def run_monitor_screen(request: TuiRenderRequest, stores: TuiStores) -> TuiScreen:
    ...
    return TuiScreen(
        ...
        panels=[
            ...
            TuiPanel(
                title="Tool Calls", items=[call.model_dump(mode="json") for call in tool_calls]
            ),
        ],
    )
...
def evidence_screen(request: TuiRenderRequest, stores: TuiStores) -> TuiScreen:
    ...
    return TuiScreen(
        ...
        panels=[
            TuiPanel(
                title="Evidence Items", items=[item.model_dump(mode="json") for item in evidence]
            )
        ],
    )
...
def verification_screen(request: TuiRenderRequest, stores: TuiStores) -> TuiScreen:
    ...
    return TuiScreen(
        ...
        summary={
            ...
            "comp_support_status": support_snapshot.get("status", "unknown"),
            "combined_support_tier": support_snapshot.get("combined_support_tier", "unknown"),
            "jurisdiction_alignment_status": _check_status(
                latest_verification, "jurisdiction_alignment"
            ),
...
def source_catalog_screen(request: TuiRenderRequest) -> TuiScreen:
    ...
    return TuiScreen(
        ...
        panels=[
            TuiPanel(
                title="South Florida GIS",
                items=[item.model_dump(mode="json") for item in gis_sources],
            ),
            TuiPanel(
                title="Municode", items=[item.model_dump(mode="json") for item in municode_sources]
            ),
            TuiPanel(
                title="Cost Assumptions",
                items=[item.model_dump(mode="json") for item in cost_sources],
            ),
        ],
    )
...
def training_screen(request: TuiRenderRequest) -> TuiScreen:
    ...
    return TuiScreen(
        ...
        panels=[
            TuiPanel(title="Video Sources", items=[item.model_dump(mode="json") for item in videos])
        ],
    )
