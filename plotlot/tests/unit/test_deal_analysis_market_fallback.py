...
def _zillow_comp(listing_type: str, price: float = 2800.0) -> dict:
    return {
        ...
        "sold_date": "2024-01-15" if listing_type == "sold" else None,
        "latitude": 26.0,
        "longitude": -80.0,
...
@pytest.mark.asyncio
async def test_zillow_comps_land_uses_sold_listings():
    ...
    with patch(
        ...
        return_value=HandlerResult(
            output_json={"comparables": [_zillow_comp("sold")], "source": "zillow", "count": 1},
            evidence_ids=[],
        ),
    ) as mock_zillow:
    ...
    assert call_args["listing_type"] == "sold"
