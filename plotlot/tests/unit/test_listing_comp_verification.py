...
def test_build_contextual_land_listing_verification_rejects_lot_sizes_that_are_not_similar() -> (
    None
):
    result = build_contextual_land_listing_verification(
        candidates=[
...
def test_build_contextual_land_listing_verification_orders_candidates_by_best_lot_fit() -> None:
    ...
    assert (
        result["verified_candidates"][0]["address_hint"]
        == "100 Close Fit Rd, Miami Gardens, FL 33056"
    )
    assert (
        result["verified_candidates"][0]["fit_score"]
        > result["verified_candidates"][1]["fit_score"]
    )
    assert (
        result["verified_candidates"][0]["lot_size_variance_ratio"]
        < (result["verified_candidates"][1]["lot_size_variance_ratio"])
    )
    assert all(
        candidate["municipality_match"] is True for candidate in result["verified_candidates"]
    )


def test_build_contextual_land_listing_verification_rejects_explicit_municipality_mismatch() -> (
    None
):
    result = build_contextual_land_listing_verification(
        candidates=[
...
def test_build_contextual_land_listing_verification_prefers_same_zip_candidate() -> None:
    ...
    assert (
        result["verified_candidates"][0]["address_hint"]
        == "100 Near Lot Rd, Miami Gardens, FL 33169"
    )
