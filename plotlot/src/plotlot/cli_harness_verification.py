...
from plotlot.harness.verification_inspection import verification_payload as build_verification_payload
    ...
    try:
        match args[0]:
            case "list":
                ...
                results = ledger.list_verifications(run_id=None if run_id is None else RunId(run_id))
                print(json.dumps({"verifications": [_verification_payload(item) for item in results]}))
                return 0
            case "show":
...
def _verification_show(args: list[str]) -> int:
    ...
    if args:
        ...
        print(json.dumps(_verification_payload(verification)))
        return 0
    print(json.dumps({"error": "usage", "usage": "plotlot verification show <verification-id|--report-id ID>"}))
    return 2
