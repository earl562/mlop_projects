...
def run_command(args: list[str]) -> int:
    if not args:
        print(
            json.dumps(
                {"error": "usage", "usage": "plotlot run acquisition-memo --address ADDRESS"}
            )
        )
        return 2
...
def runs_command(args: list[str]) -> int:
    ...
    try:
        match args[0]:
            case "list":
                print(
                    json.dumps({"runs": [run.model_dump(mode="json") for run in store.list_runs()]})
                )
                return 0
            ...
            case "events" if len(args) >= 2:
                events = [
                    event.model_dump(mode="json") for event in store.get_events(RunId(args[1]))
                ]
