...
from plotlot.harness.report_store import (
    ClaimNotFoundError,
    ReportNotFoundError,
    default_report_ledger,
)
...
def reports_command(args: list[str]) -> int:
    if not args:
        print(
            json.dumps({"error": "usage", "usage": "plotlot reports <list|show|export|finalize>"})
        )
        return 2
    ledger = default_report_ledger()
    try:
        match args[0]:
            case "list":
                ...
                print(
                    json.dumps({"reports": [report.model_dump(mode="json") for report in reports]})
                )
                return 0
            case "show" if len(args) >= 2:
        ...
        verification_id = (
            None if exc.verification is None else str(exc.verification.verification_id)
        )
        print(
            json.dumps(
                {
