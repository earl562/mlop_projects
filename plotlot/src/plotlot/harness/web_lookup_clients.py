    ...
    *,
    ...
    if resolved_provider is None:
        return WebSearchResult(
            ...
            provider=normalized_provider,
            message=(f"Web search is not available (set EXA_API_KEY). {_DEFAULT_GUIDANCE}"),
        )
...
def _result_items(
    ...
    *,
    ...
    return [
        _result_item(item, provider=provider) for item in raw_items[:5] if isinstance(item, dict)
    ]
