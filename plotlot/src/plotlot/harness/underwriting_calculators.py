...
def run_brrrr_refinance_analysis(request: BRRRRRefinanceInput) -> BRRRRRefinanceResult:
    ...
    if cash_left <= 0:
        warnings.append(
            "Refinance proceeds recover all project cost before reserves and closing friction."
        )
    return BRRRRRefinanceResult(
        ...
        else round(request.annual_noi / annual_debt_service, 4),
...
def run_feasibility(request: FeasibilityInput) -> FeasibilityResult:
    ...
    if not area_limiters:
        warnings.append(
            "No FAR, lot coverage, or setback envelope supplied; buildable area is zero."
        )
    ...
    return FeasibilityResult(
        ...
        if buildable_envelope_sf is None
        else round_money(buildable_envelope_sf),
