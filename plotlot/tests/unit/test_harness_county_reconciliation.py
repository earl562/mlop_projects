...
def test_county_reconciliation_accepts_missing_direction_when_street_number_and_type_align() -> (
    None
):
    assert _listing_candidate_matches_county_record(
        CountyReconciliationCandidate(
...
async def test_live_run_rejects_county_reconciliation_when_sale_dates_do_not_align(
    ...
    assert (
        reconciliation["rejected_candidates"][0]["reason"]
    ...
    assert not any(
        comp.get("verification_status") == "county_reconciled" for comp in public_listing_comps
    )
