...
def south_florida_address_paths_suite() -> EvalResult:
    ...
    for case_name, address, expected_county in fixture_addresses:
        ...
        if (
            ...
            or property_record.get("county") != expected_county
        ):
...
def _run_live_address_path_case(case: _LiveAddressPathCase) -> EvalCaseResult:
    ...
    if not isinstance(property_record, dict):
        ...
        if (
            str(property_record.get("municipality") or "").strip().casefold()
            != case.municipality.casefold()
        ):
            failures.append("property municipality mismatch")
        if (
            str(property_record.get("zoning_code") or "").strip().upper()
            != case.zoning_code.upper()
        ):
...
_MANUAL_LAND_COMP_QUALITY: JsonObject = {
    "land_comp_count": 2,
...
def _manual_offer_eval_case(case: _ManualOfferEvalCase) -> EvalCaseResult:
    ...
    if not {
        ...
        failures.append(
            "live manual-offer path no longer covers parcel, zoning, comps, and underwriting"
        )
    for key, expected_value in case.expected_feasibility.items():
        actual_value = observed_feasibility_args.get(key)
        if isinstance(expected_value, float):
            if not isinstance(actual_value, int | float) or not isclose(
                float(actual_value), expected_value, abs_tol=0.1
            ):
    ...
    if not isclose(
        float(observed_pro_forma_args.get("estimated_land_value") or 0.0), case.expected_land_value
    ):
        failures.append(f"{case.name} pro forma estimated_land_value changed")
    if not isclose(
        float(observed_pro_forma_args.get("adv_per_unit") or 0.0), case.expected_adv_per_unit
    ):
    ...
    if not isinstance(manual_comparables, dict) or not _matches_manual_land_comp_quality(
        manual_comparables.get("land_comp_quality")
    ):
    ...
    if (
        ...
        or bool(guidance.get("land_comp_signal_available"))
        is not case.expected_land_signal_available
    ):
    ...
    if not isclose(
        float(guidance.get("recommended_offer_low") or 0.0), case.expected_guidance_offer
    ):
        failures.append(f"{case.name} acquisition guidance recommended_offer_low changed")
    if not isclose(
        float(guidance.get("recommended_offer_high") or 0.0), case.expected_guidance_offer
    ):
    ...
    if (
        not isinstance(guidance, dict)
        or guidance.get("owner_basis_warning") != case.expected_owner_basis_warning
    ):
    ...
    if case.expected_manual_dimensional is not None:
        ...
        for key, expected_value in case.expected_manual_dimensional.items():
            actual_value = (
                manual_dimensional.get(key) if isinstance(manual_dimensional, dict) else None
            )
            if not isinstance(actual_value, int | float) or not isclose(
    ...
    if case.expected_gis_warning is not None:
        ...
        if (
            not isinstance(gis_site_context, dict)
            or gis_site_context.get("warning") != case.expected_gis_warning
        ):
            failures.append(f"{case.name} Broward GIS context warning changed")
    comp_claim = next(
        (claim for claim in result.claims if claim.claim_type == "comp_value_signal"), None
    )
    ...
    elif not _matches_manual_land_comp_quality(comp_claim.metadata.get("land_comp_quality")):
        failures.append(f"{case.name} comp value claim land comp quality changed")
    land_comp_evidence = next(
        (
            item
            for item in result.evidence_items
            if item.source_url.endswith("-land-1") or item.source_url.endswith("land-1")
        ),
        None,
    )
    if (
        land_comp_evidence is None
        or land_comp_evidence.metadata.get("comp_quality_status") != "user_supplied_unscored"
    ):
        failures.append(f"{case.name} manual land comp evidence quality changed")
    if (
        land_comp_evidence is None
        or land_comp_evidence.metadata.get("manual_override_used") is not True
    ):
...
def _matches_manual_land_comp_quality(value: object) -> bool:
    return isinstance(value, dict) and all(
        value.get(key) == expected for key, expected in _MANUAL_LAND_COMP_QUALITY.items()
    )
...
def _manual_offer_eval_cases() -> tuple[_ManualOfferEvalCase, _ManualOfferEvalCase]:
    return (
        _ManualOfferEvalCase(
            ...
            expected_manual_dimensional={
                "max_height_ft": 35.0,
                "max_stories": 2.0,
                "min_lot_frontage_ft": 75.0,
                "water_setback_ft": 0.0,
                "accessory_separation_ft": 10.0,
            },
        ...
        _ManualOfferEvalCase(
            ...
            noi_result={
                ...
                "operating_expenses": 12768.0,
                "annual_noi": 27360.0,
                "as_built_value": 480000.0,
                "warnings": [],
            },
            residual_result={
                "calculation_type": "residual_land_value",
                "formula_version": "residual_land_value.v1",
                "total_project_costs_excluding_land": 300000.0,
                "max_supportable_land_price": 180000.0,
