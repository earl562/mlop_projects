...
_BROWARD_CODE_TO_CITY: dict[str, str] = {value: key.title() for key, value in BROWARD_CITY_CODES.items()}
_DIRECTION_TOKENS = frozenset({"N", "S", "E", "W", "NE", "NW", "SE", "SW"})
_STREET_TYPE_TOKENS = frozenset({"BLVD", "AVE", "ST", "DR", "CT", "LN", "PL", "RD", "TER", "WAY", "CIR"})
...
def _build_broward_where_clause(
    *,
    ...
    if street_name:
        ...
        value = (
            f"'{_escape_where(street_name)}'"
            if exact_name
            else f"'%{_escape_where(street_name)}%'"
        )
...
def _addresses_confidently_match(candidate: str, target: str) -> bool:
    candidate_number, candidate_direction, candidate_name, candidate_type = _address_components(candidate)
...
def _addresses_spatially_recover(candidate: str, target: str) -> bool:
    ...
    if not candidate_number or not target_number or not candidate_name or not target_name:
        return False
    if candidate_direction != target_direction or candidate_type != target_type or candidate_name != target_name:
        return False
...
async def _lookup_miami_dade(
    ...
    with start_span(name="lookup_miami_dade", span_type="TOOL") as span:
        ...
        if not _addresses_confidently_match(str(best_feature.get("attributes", {}).get("TRUE_SITE_ADDR") or ""), address):
            spatial_fallback = await _lookup_miami_dade_from_property_boundary(
                address=address,
                lat=lat,
                lng=lng,
...
async def _lookup_miami_dade_from_property_boundary(
    *,
    ...
    parcel_features = await _query_arcgis(
        ...
        where="1=1",
        out_fields=(
            "FOLIO,TRUE_SITE_ADDR,TRUE_SITE_CITY,TRUE_OWNER1,LEGAL,Shape.STArea()"
        ),
        extra_params={
            "geometry": f"{lng},{lat}",
            "geometryType": "esriGeometryPoint",
    ...
    if not zoning_code or zoning_code.upper() == "NONE":
        zoning_code, zoning_desc = await _spatial_query_zoning(MDC_UNINCORPORATED_ZONING_URL, lat, lng)
...
async def _lookup_broward(
    ...
    with start_span(name="lookup_broward", span_type="TOOL") as span:
        ...
        if remaining and remaining[0] in {"N", "S", "E", "W", "NE", "NW", "SE", "SW"}:
            ...
            remaining = remaining[1:]
        street_type = ""
        if remaining and remaining[-1] in {"BLVD", "AVE", "ST", "DR", "CT", "LN", "PL", "RD", "TER", "WAY", "CIR"}:
            street_type = remaining[-1]
            remaining = remaining[:-1]
        street_name = " ".join(remaining).strip()
            ...
            try:
                ...
                if record:
                    if record.address:
                        ...
                        spatially_recovered = record.lot_size_source == "geometry" and _addresses_spatially_recover(
                            record.address,
                            address,
                        )
                ...
                if _is_transient_property_lookup_error(e) and attempt < PROPERTY_LOOKUP_MAX_ATTEMPTS:
