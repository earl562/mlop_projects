...
_SLASH_DATE_PATTERN: Final[re.Pattern[str]] = re.compile(
...
_FL_ZIP_CODE_PATTERN: Final[re.Pattern[str]] = re.compile(
    r"\bFL\s+([0-9]{5})(?:-[0-9]{4})?\b", re.IGNORECASE
)
...
def extract_zip_code(*values: object) -> str | None:
    for value in values:
        ...
        if all_matches:
            zip_code = str(all_matches[-1]).strip()
            if zip_code:
                return zip_code
    return None


def zip_matches_subject(
    *, subject_zip_code: str | None, candidate_zip_code: str | None
) -> bool | None:
...
def municipality_matches_subject(
    *, subject_municipality: str, candidate_municipality: str | None
) -> bool:
...
def _largest_sqft_match(raw_text: str) -> float | None:
    ...
    if not matches:
        return None
    values = [
        value for match in matches if (value := _normalized_number(match.group(1))) is not None
    ]
