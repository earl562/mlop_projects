...
async def test_fort_lauderdale_manual_land_offer_flow_uses_verified_standard_and_contextual_broward_gis(
    ...
    assert any(
        tool_call["tool_name"] == "run_residual_land_value" for tool_call in data["tool_calls"]
    )
    comp_claim = next(
    ...
    assert claim_land_comp_quality["best_fit_lot_size_variance_ratio"] == pytest.approx(
        0.083, abs=0.001
    )
    ...
    land_comp_evidence = next(
        ...
        if item["source_url"] == "https://example.test/ftl-land-1"
    )
    ...
    exit_comp_evidence = next(
        item
        for item in data["evidence_items"]
        if item["source_url"] == "https://example.test/ftl-exit-1"
    )
