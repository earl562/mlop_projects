...
def query_gis_feature_service(
    ...
    *,
    ...
    if source_mode is SourceMode.LIVE:
        raise RuntimeError(
            "live GIS feature service queries must use query_gis_feature_service_async"
        )
    features = fixture_features(source_id)[:limit]
    return GISFeatureQueryResult(
        source_id=source_id,
        provider=GISProvider(source.provider),
        features=features,
    )
...
def _create_gis_evidence(
    ...
    freshness = (
        FreshnessStatus.UNKNOWN if source_mode is SourceMode.LIVE else FreshnessStatus.FIXTURE
    )
    return EvidenceItem(
