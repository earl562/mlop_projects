...
async def municode_search(body: MunicodeSearchRequest) -> JsonObject:
    ...
    return {"source_mode": body.source_mode.value, "results": [item.model_dump(mode="json") for item in results]}


@router.get("/municode/sections/{section_id}")
async def municode_section(section_id: str, source_mode: SourceMode = SourceMode.FIXTURE) -> JsonObject:
    try:
