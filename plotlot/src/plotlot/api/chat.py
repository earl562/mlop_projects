...
# The grounded payload and its two helpers moved to the pipeline layer so the harness
# can serve MCP/CLI tool calls without importing this API module. Re-exported in the
# explicit `as` form (PEP 484) because chat itself and a large body of tests still
# import them from this path.
from plotlot.pipeline.grounding import _format_grounded_analysis as _format_grounded_analysis
from plotlot.pipeline.grounding import _format_sensitivity as _format_sensitivity
from plotlot.pipeline.grounding import _round as _round
...
def _build_active_analysis_context(payload: dict) -> str:
    ...
    #
    # There are THREE states, and the undetermined one is the dangerous one. Since
    # `2a2f71c` the payload emits no district at all (zoning_code=None) when no GIS
    # layer supplied one, so a re-render that only prints `Zoning: {code}` renders an
    # EMPTY field carrying no prohibition — exactly the condition under which the
    # narrator has historically supplied a district of its own invention.
    zoning_status = payload.get("zoning_status") or ""
    zoning_src = payload.get("zoning_source") or ""
    zoning_txt = payload.get("zoning_code", "")
    if zoning_status == "not_determined" or (zoning_status and not zoning_txt):
        zoning_txt = "NOT DETERMINED (no parcel/zoning GIS layer covers this parcel)"
    elif zoning_txt and zoning_src == "gis":
        zoning_txt = f"{zoning_txt} (parcel/zoning GIS layer)"
    elif zoning_txt:
        # Legacy path: only reachable for a payload cached before districts stopped
        # being inferred. Kept so an old session degrades loudly rather than silently.
        zoning_txt = (
    ...
    # The prohibition and the searched-candidate name must both survive into every
    # follow-up turn. Without them the model sees only a blank district and fills it.
    if zoning_status == "not_determined":
        if payload.get("zoning_basis"):
            lines.append(f"  ZONING: {payload['zoning_basis']}")
        candidate = payload.get("unverified_district_candidate")
        if candidate:
            lines.append(
                f"  The ordinance search was run against {candidate!r} as a CANDIDATE ONLY. "
                "It is not this parcel's district — never cite it as one."
            )
    ...
    if by_right:
        ...
        # PROVISIONAL alone is a label the narrator can shrug off; the *reason* is
        # what makes it repeatable to the user. The reasons are computed
        # deterministically in pipeline/trust.py and echoed verbatim here — the
        # narrator layer must not re-derive them from the boolean flags, or the two
        # wordings drift apart exactly the way the payload and this block already did.
        reasons = by_right.get("provisional_reasons") or []
        if verif == "provisional" and reasons:
            lines.append(
                "  PROVISIONAL because: " + "; ".join(reasons) + ". "
                "State this whenever you give the number."
            )
    ...
    # Measured slope. This is a deterministic 3DEP measurement, not a model opinion,
    # and it is the reason a hillside parcel's count is an upper bound — so it has to
    # persist across turns exactly like the lot-size provenance does.
    # The wording comes from TerrainAnalysis.summary()/yield_caveat(), which are
    # deterministic and already carry the ESL consequence for a steep hillside. Echo
    # them rather than composing a regulatory claim here.
    terrain = payload.get("terrain") or {}
    if terrain:
        lines.append(f"Terrain (measured, USGS 3DEP): {terrain.get('summary', '')}")
        if payload.get("buildable_area_basis"):
            lines.append(f"  {payload['buildable_area_basis']}")
    ...
    # Trust caveats are already rendered structurally above (zoning basis, the
    # PROVISIONAL reason list, terrain) where nothing can truncate them. Subtract
    # them here so they neither duplicate nor consume the 4 slots this cap allows —
    # the cap exists to bound a prompt block injected on every single turn.
    trust_caveats = set(payload.get("trust_caveats") or [])
    general_warnings = [w for w in (payload.get("warnings") or []) if w not in trust_caveats]
    for w in general_warnings[:4]:
