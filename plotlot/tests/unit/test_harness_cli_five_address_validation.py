...
def test_cli_validates_five_miami_dade_and_broward_addresses(
    ...
    assert payload["artifacts"]["comp_search_strategy"]["land_signal_tier"] == case.expected_land_signal_tier
    ...
    assert payload["artifacts"]["acquisition_guidance"]["recommended_action"] == case.expected_action
    ...
    assert "public_listing_comps" not in {section["section_id"] for section in payload["report"]["sections"]}
