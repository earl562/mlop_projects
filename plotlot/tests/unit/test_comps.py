...
def _keep_miami_dade_comp_tests_offline(monkeypatch):
    async def passthrough(features):
        return features

    monkeypatch.setattr(
        "plotlot.pipeline.comps._enrich_miami_dade_sales_features",
        passthrough,
    )
    ...
    def test_filters_land_sales_above_finished_home_pricing_band(self):
        ...
        land_comps = [
            ComparableSale(
                address="High Lot",
                sale_price=900000.0,
                lot_size_sqft=11250.0,
                price_per_acre=3484800.0,
            ),
            ComparableSale(
                address="Reasonable Lot",
                sale_price=145000.0,
        ...
        filtered, rejected = _filter_vacant_single_family_land_comps(
            subject, land_comps, unit_comps
        )
    ...
    def test_returns_empty_when_all_land_sales_exceed_finished_home_pricing_band(self):
        ...
        land_comps = [
            ...
            ComparableSale(
                address="High Lot 2",
                sale_price=725000.0,
                lot_size_sqft=11025.0,
                price_per_acre=2864489.8,
            ),
        ]
        unit_comps = [
            ComparableSale(address="Exit 1", sale_price=505000.0, price_per_unit=505000.0),
...
class TestComparableFallbacks:
    async def test_find_comparables_for_vacant_single_family_subject_avoids_unreliable_land_fallbacks(
        self,
    ):
        from unittest.mock import AsyncMock, patch
        ...
        with (
            ...
            patch(
                "plotlot.pipeline.comps._query_nearby_sales", new=AsyncMock(return_value=features)
            ),
        ):
        ...
        assert any(
            "No recent same-market new-build sales were available" in note for note in result.notes
        )
    ...
    async def test_find_comparables_rejects_oversized_recent_new_build_outliers_for_vacant_single_family_subject(
        self,
    ):
        from unittest.mock import AsyncMock, patch
    ...
    async def test_find_comparables_prefers_nearby_local_exit_cluster_for_vacant_single_family_subject(
        self,
    ):
        from unittest.mock import AsyncMock, patch
    ...
    async def test_find_comparables_prefers_nearby_local_exit_cluster_over_premium_new_build_micro_market(
        self,
    ):
        from unittest.mock import AsyncMock, patch
        ...
        assert any("higher-priced nearby micro-market" in note for note in result.notes)

    async def test_find_comparables_prefers_nearby_local_land_cluster_for_vacant_single_family_subject(
        self,
    ):
        from unittest.mock import AsyncMock, patch
    ...
    async def test_find_comparables_enriches_broward_sales_with_property_info(self):
        ...
        assert result.adv_per_unit == 425000.0
        assert any(
            "lower-confidence fallback improved sales outside the exact zoning filter"
            in note.lower()
            for note in result.notes
        )

    async def test_find_comparables_uses_lower_confidence_fallback_when_strict_land_filters_fail(
        self,
    ):
        from unittest.mock import AsyncMock, patch
        ...
        with (
            patch(
                ...
                new=AsyncMock(
                    return_value=(
                        "https://example.test/sales",
                        ["SALE_PRICE", "SALE_DATE", "ADDRESS", "LOT_SIZE", "ZONING"],
                    )
                ),
            ),
            patch(
    ...
    async def test_find_comparables_uses_lower_confidence_fallback_when_only_relaxed_improved_sales_exist(
        self,
    ):
        from unittest.mock import AsyncMock, patch
    ...
    async def test_find_comparables_does_not_treat_unknown_broward_metadata_as_direct_match(self):
        ...
        features = [
            ...
            },
            {
                ...
                "geometry": {
                    "rings": [
                        [[-80.161, 26.145], [-80.160, 26.145], [-80.160, 26.146], [-80.161, 26.145]]
                    ]
                },
            },
        ]
    ...
    async def test_find_comparables_promotes_relaxed_vacant_lot_land_comps_when_county_data_is_strong(
        self,
    ):
        from unittest.mock import AsyncMock, patch
        ...
        with (
            patch(
                ...
                new=AsyncMock(
                    return_value=(
                        "https://example.test/sales",
                        ["FOLIO_NUMBER", "SALE_AMOUNT", "SALE_DATE"],
                    )
                ),
            ),
            patch(
