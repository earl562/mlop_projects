...
async def _handle_lookup_property_info(
    ...
    if _is_fixture_address(address):
        ...
        try:
            ...
            return {
                "status": "error",
                "message": f"Property lookup failed: {type(e).__name__}: {e}",
            }
        if not record:
            return {"status": "not_found", "result": {}}
    ...
    property_payload: dict[str, Any] = {
        ...
        "lot_size_source": record.lot_size_source,
...
def _ordinance_authority_metadata(
    *,
    ...
    jurisdiction = ", ".join(part for part in (municipality.strip(), state.strip().upper()) if part)
    return {
...
async def _handle_search_municode_live(
    ...
    payload: dict[str, Any] = {
        ...
        **_ordinance_authority_metadata(
            ...
            resolution="municode_targeted_discovery",
...
async def _handle_generate_document(args: dict[str, Any], _context: ToolContext) -> dict[str, Any]:
...
async def _handle_get_dataset_info(_args: dict[str, Any], context: ToolContext) -> dict[str, Any]:
...
async def _handle_create_spreadsheet(args: dict[str, Any], _context: ToolContext) -> dict[str, Any]:
