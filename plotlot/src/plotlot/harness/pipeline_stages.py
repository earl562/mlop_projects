...
def _zoning_stage(artifacts: JsonObject, warnings: list[str]) -> PipelineStageSummary:
    ...
    status = "completed" if ordinance else ("partial" if source_context or zoning_code else "missing")
...
def _feasibility_stage(
    ...
    if feasibility:
        ...
        status = "warning" if _warning_matches(warnings, "feasibility", "maxfar", "maxunits") else "missing"
    else:
        status = "not_required"
...
def _underwriting_stage(
    ...
    if noi and residual:
        ...
        status = "warning" if _warning_matches(warnings, "noi", "residual", "underwriting", "rent") else "missing"
    else:
        status = "not_required"
