...
def _ev(type_: HarnessEventType, run_id: str, severity: str, payload: dict) -> HarnessEvent:
    return HarnessEvent(type=type_, severity=severity, payload=payload,
...
async def start_run(
    ...
    *,
    ...
    events.append(_ev(HarnessEventType.RUN_REQUESTED, run_id, "info", {
        "analysis_run_id": run_id, "skill_name": req.skill_name,
        "site_id": req.site.get("address", ""), "intended_use": req.intended_use,
    }))
    events.append(_ev(HarnessEventType.RUN_STARTED, run_id, "info",
    ...
    if zoning_code:
        ...
        claims.append(ReportClaim(
            key="zoning.district", text=f"Zoning district: {zoning_code}",
            material=True, evidence_ids=[ev_id], confidence="high",
            source_caveat="Online code may not be official; verify with municipality.",
        ))
        events.append(_ev(HarnessEventType.EVIDENCE_RECORDED, run_id, "info", {
            "evidence_id": ev_id, "tool_run_id": "zoning_lookup",
            "claim_key": "zoning.district", "source_type": "gis_zoning",
        }))
    else:
        # No evidence → unknown, needs_verification (master spec rule #11).
        claims.append(ReportClaim(
            key="zoning.district", text="Zoning district unknown / requires verification",
            material=False, evidence_ids=[], confidence="unknown",
    ...
    events.append(_ev(HarnessEventType.REPORT_COMPLETED, run_id, "info", {
        "report_id": report["report_id"], "analysis_run_id": run_id,
        "claim_count": len(report["claims"]), "uncited_count": report["uncited_count"],
    }))

    status = "completed" if report["uncited_count"] == 0 else "needs_review"
    events.append(_ev(HarnessEventType.RUN_COMPLETED, run_id, "info", {
        "analysis_run_id": run_id, "report_id": report["report_id"], "duration_ms": 0,
    }))

    return RunResult(
        analysis_run_id=run_id, status=status, report_id=report["report_id"],
        evidence_ids=evidence_ids, report_claims=claims, events=events,
    )
